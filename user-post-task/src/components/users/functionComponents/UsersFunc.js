...
const UsersFunc = () => {
    ...
    const search = (rows) => {
        return rows.filter((row) => row.name.toLowerCase().includes(query.trim().toLowerCase())
            || row.username.toLowerCase().includes(query.trim().toLowerCase()) || row.email.toLowerCase().includes(query.trim().toLowerCase())
        );
    }
    ...
    return (
        <>
            <div>
                {!isAddData && <Button onClick={startAddDataHandler}>Add New User</Button>}
                {isAddData && <AddDataForm onSaveUserData={saveUserDataHandler} onCancel={stopAddDataHandler} />}
            </div>
            <br />
            <div>
                {isEditing ? <EditUser editUser={editUserData} onCancel={stopEditData} onEditSuccess={updateDataHandler} /> : null}
            </div>
