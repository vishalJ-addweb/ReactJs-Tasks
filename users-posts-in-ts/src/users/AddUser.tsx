...
const AddDataFormFunc = (props: any) => {

    const [add, setAdd] = useState<AllStatesOfAddUser>(defaultAddUserStateValues);

    const submitHandler = (event: any) => {
        event.preventDefault();
        if (add.name && add.userName && add.email && add.street && add.suite && add.city && add.zipCode && add.lat && add.lng && add.phone && add.website && add.compName && add.catchPhrase && add.bs) {
            const userData = {
                name: add.name,
